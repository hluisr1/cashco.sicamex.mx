$(document).ready(function () {
    ...
    */
	...
	var tabla = 'Estatus de Visita';
	var MsjValida = ['Ingrese el Nombre del ' + tabla];
	
	
	var FRM = new CRUD('12-ESTATUS.VISITA', {C: '#CRUD_C', R: '.CRUD_R', U: '#EDITA', D: '#ELIMINA'}, INI_PAGE);
	...
	window.i_Reg = function (nombre) {
		...
		if(fErr == -1){
			...
			$.ajax({
                type: "POST",
                url: domin + "/fWS.asmx/i_EstatusVisita",
    ...
    window.u_Reg = function (id_, nombre) {
		...
		if(fErr == -1){
			...
			$.ajax({
                type: "POST",
                url: domin + "/fWS.asmx/u_EstatusVisita",
                data: "{'CompanyCode':'"+company+"',"+ 
                	"id: '"+id_+"'," +
					"Descripcion: '"+nombre.trim()+"',"+
					"idUsuario: '"+getUserSession().Nickname+"'}",
    ...
    window.d_Reg = function (id_) {
		...
		$.ajax({
            type: "POST",
            url: domin + "/fWS.asmx/d_EstatusVisita",
            data: "{'CompanyCode':'"+company+"',"+ 
            	"idUsuario: '"+getUserSession().Nickname+"',"+
            	"id: '"+id_+"'}",
            contentType: "application/json; charset=utf-8",
